...
for idx, model in enumerate(models):
    ...
    with open(f"{base_dir}/{filename}") as file:
        v, vt, vn, f = parseobj(file)
...
def generate_camera_ray(sampler, resolution):
    ...
    pixel = ((make_float2(coord) + sampler.next2f()) * 2.0 - float2(
        resolution)) / frame_size  # remapped to [-1,1] in shorter axis
...
gui = luisa.GUI("PyLuisaRender", resolution=resolution)
frame_id = 0
while gui.running():
